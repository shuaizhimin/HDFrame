...
package com.handsome.frame.android.volley;
...
import com.handsome.frame.android.volley.NetroidLog.MarkerLog;
/**
...
public abstract class Request<T> implements Comparable<Request<T>> {
	...
	/** Listener interf for response and error. */
	...
	public Request(int method, String url, Listener<T> listener) {
		if (HandsomeApplication.DEBUG) {
			...
			if (url.startsWith("http://")) {
				...
				mUrl = HandsomeApplication.remoteURL + url;
			}
		}
		HDLog.d(TAG + "-URL:", mUrl);
		mMethod = method;
		mListener = listener;
	...
	/**
	 * Set how long the cache is expired, {@link com.handsome.android.frame.volley.cache}
	 ...
	 *
	 * <p>
	 * Note that you can directly override {@link #} for custom data.
	 * </p>
	 *
	 * @throws AuthFailureError
	 *             in the event of auth failure
	...
	public static byte[] encodeParameters(Map<String, String> params,
		...
		HDLog.d(TAG + "-入参：", JSON.toJSONString(params));
		...
		if (HandsomeApplication.DEBUG) {
			...
			try {
				InputStreamReader inputReader = new InputStreamReader(
						HandsomeApplication.mContext.getAssets().open(mUrl));
